...
public class SlidingMenu extends ViewGroup {
    ...
    private boolean inteceptEvent;

    //true,表示不要父控件拦截
    public void setInterceptEvent(boolean inteceptEvent) {
        this.inteceptEvent = inteceptEvent;
    }
...
//                intercept = super.onInterceptTouchEvent(ev);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(deltaX) > mTouchSlop && Math.abs(deltaX) > Math.abs(deltaY)) {//横向滑动
                    intercept = true;
                } else {//纵向滑动
                    intercept = false;
                }
