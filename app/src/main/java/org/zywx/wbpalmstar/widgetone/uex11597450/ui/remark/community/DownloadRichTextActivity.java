...
public class DownloadRichTextActivity extends BaseActivity {
    ...
    protected void asyncUiInfo() {
        addToCompositeDis(HttpUtil.getDownloadDeail(postId)
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                    public void accept(@NonNull Disposable disposable) throws Exception {
                        showLoadDialog();
                ...
                .subscribe(new Consumer<DownloadDetailData>() {
                    @Override
                    public void accept(@NonNull DownloadDetailData data) throws Exception {
                        dismissLoadDialog();
                        refreshUi(data);
                    }
                }, new Consumer<Throwable>() {
    ...
    private void refreshUi(DownloadDetailData data) {
    ...
    private void goDownloadCondi() {
        if (GlobalUser.getInstance().isAccountDataInvalid()) {
            //登录无效。
            new LoginTipDialog().showDialog(getSupportFragmentManager());
        } else {
            //需要评论
